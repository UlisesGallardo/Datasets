...
#  Recomendation System Analysis Trainig Corpus
-*- coding: utf-8 -*-

## Spanish (Español)

El conjunto de datos se encuentra dividido en 2 archivos: rest-mex_2022_recommendation_data_training.csv, Places.csv y una carpeta llamada UsersTraining

#rest-mex_2022_recommendation_data_training.csv:
El conjunto de datos consta de 1582 usuarios (el 70 % del conjunto de datos original. El otro 30 % será utilizado como conjunto de prueba), uno por cada recomendación que recibió un turista. Cada renglón en el archivo contiene 7 columnas:
...
La etiqueta va de 1, que significa el mayor grado de insatisfacción, hasta 5 que es el mayor grado de satisfacción. Se puede interpetar de la siguiente manera:
1. Muy malo
2. Malo
3. Neutral
4. Bueno
...
#UsersTraining
La carpeta contiene 1351 archivos, cada archivo fue nombrado como 'UsuarioN.csv' donde UsuarioN es el índice para que se pueda identificar dentro del archivo  rest-mex_2022_recomendation_data_training.csv. Cara archivo csv tiene una lista del historial de opiniones que ha dado el UsuarioN en TripAdvisor. Estos datos estan organizados en 4 columnas donde en cada renglón se puede encontrar la siguiente información:
...
The data set is divided into two files: rest-mex_2022_recommendation_data_training.csv, Places.csv, and a folder called UsersTraining

# rest-mex_2022_recommendation_data_training.csv:
The data set consists of 1582 users (70% of the original data set. The other 30% will be used as a test set), one for each recommendation received by a tourist. Each row in the file contains seven columns:
...
7. Label: The label representing the level of satisfaction that the tourist will have when going to the recommended place. Data type: [1, 2, 3, 4, 5]
The label ranges from 1, which means the highest degree of dissatisfaction, to 5, which is the highest degree of satisfaction. It can be interpreted as follows:
1. Very bad
2. Bad
3. Neutral
4. Good
...
#UsersTraining
The folder contains 1351 files; each file was named 'UserN.csv' where UserN is the index to be identified within the rest-mex_2021_recomendation_data_training.csv file. Each csv file has a list of the review history that UserN has given on TripAdvisor. These data are organized in 4 columns where the following information can be found in each line:
